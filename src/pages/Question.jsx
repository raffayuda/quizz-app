  ...
  return (
    <div className="h-full relative p-[54px] flex flex-col gap-[54px]">
      <>
        ...
        <div className="grid grid-cols-2 gap-x-[142px] gap-y-[42px] mx-[20px]">
          {Object.entries(questions[0].options).map(([key, value]) => (
            <div
              key={key}
              className="bg-amber-700 rounded-[12px] px-[42px] py-[24px] text-white text-lg text-center hover:bg-slate-500 transition-all duration-300 cursor-pointer"
            >{`${key}. ${value}`}</div>
          ))}
        </div>
      </>
      ...
      <svg
                ...
                <div className="text-[36px] bg-amber-700 flex items-center p-1 text-pink-400 border-[6px] border-pink-400 hover:scale-110 hover:text-white hover:border-white">Finish</div>
      </div>
    </div>
  );
};
