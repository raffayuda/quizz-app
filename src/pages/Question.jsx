...
const Question = ({ index, answer, setAnswer, handleNext, handlePrev, setScore, setDisplay }) => {
        ...
        <div className="grid grid-cols-2 gap-x-[142px] gap-y-[42px] mx-[20px]">
          {Object.entries(questions[index].options).map(([key, value]) => (
            <div
              ...
              key={key}
              className={`bg-amber-700 rounded-[12px] px-[42px] py-[24px] text-white text-lg text-center hover:bg-slate-500 transition-all duration-300 cursor-pointer ${answer[index + 1] === key ? 'bg-slate-500' : ''}`}
            >{`${key}. ${value}`}</div>
          ))}
        </div>
      </>
      <div className="absolute bottom-[20px] right-[54px] flex justify-end mx-[20px] gap-5">
        {index !== 0 && (
          <svg
            onClick={handlePrev}
            ...
            height="73"
            viewBox="0 0 72 73"
            className="bg-amber-700 rounded-full border-solid border-[6px] border-pink-400 fill-pink-400 hover:border-white hover:scale-110 hover:shadow-2xl hover:fill-white cursor-pointer"
            fill="#EAB2A0"
          >
        ...
        {index + 1 !== questions.length && <svg
          ...
          height="73"
          viewBox="0 0 72 73"
          className="bg-amber-700 rounded-full border-solid border-[6px] border-pink-400 fill-pink-400 hover:border-white cursor-pointer hover:scale-110 hover:shadow-2xl hover:fill-white"
          fill="#EAB2A0"
        >
        ...
        {index + 1 === questions.length && <div onClick={handleSubmit} className="text-[36px] bg-amber-700 flex items-center p-1 text-pink-400 border-[6px] border-pink-400 hover:scale-110 hover:text-white hover:border-white">
          Finish
        </div>}
      </div>
    </div>
